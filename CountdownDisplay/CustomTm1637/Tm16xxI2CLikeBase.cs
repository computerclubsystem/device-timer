...
namespace DeviceTimer.CountdownDisplay.CustomTm1637
{
    ...
    public abstract class Tm16xxI2CLikeBase : Tm16xxBase, IDisposable
    {
        ...
        private protected virtual void StartTransmission()
        {
            ...
            // TODO: Align to Python sample code
        ...
        private protected virtual void WriteDataCommand()
        {
            StartTransmission();
            WriteByteAndWaitAcknowledge(0x40);
            StopTransmission();
        }

        private protected virtual void WriteDisplayControl() {
            StartTransmission();
            WriteByteAndWaitAcknowledge((byte)(0x88 + 7)); // this._brightness));
            StopTransmission();
        }
        ...
        private protected virtual void WriteByte(byte data)
        {
            ...
            // self.request.set_value(self.clk, Value.INACTIVE)
            // sleep(TM1637_DELAY)
            // self.request.set_value(self.clk, Value.ACTIVE)
            // sleep(TM1637_DELAY)
            // self.request.set_value(self.clk, Value.INACTIVE)
